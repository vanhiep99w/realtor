...
function SignUp(props) {
  ...
  const onSubmit = async ({emailAddress, password, name}) => {
    try {
      ...
      const userMetaData = {username: name, emailAddress: emailAddress, timestamp: serverTimestamp()}
    ...
    <section>
      ...
      <div className="p-5">
        <div className="flex mx-auto lg:max-w-6xl space-y-10 lg:space-x-20 flex-col lg:flex-row">
          ...
          <form className="w-full flex-1 flex flex-col space-y-5" onSubmit={handleSubmit(onSubmit)}>
            <input type="text" className="input" placeholder="Full name" {...register("name")}/>
